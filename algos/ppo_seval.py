...
def _eval_policy_safety(env, policy, n_episodes=100):
    cost_returns = []
    for _ in range(n_episodes):
        obs = env.reset()
        done, ep_cost = False, 0.0
        while not done:
            action, _ = policy.predict(obs, deterministic=True)
            obs, reward, done, info = env.step(action)
            if isinstance(info, dict):
                ep_cost += info.get("cost", 0.0)  # environment should report cost
        cost_returns.append(ep_cost)
    avg_cost = sum(cost_returns) / len(cost_returns)
    return avg_cost, cost_returns
